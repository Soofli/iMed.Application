...
public class PopUpUtilities
{
    ...
    {
        PopupNavigation.Instance.Popped += (sender, e) =>
        {
            var name = e.Page.Content.GetType().Name;
            if(name == "VideoPopUp")
    ...
    public async void PushNotify(string message)
    {
        await PopAsync();
        //await PushAsync(new NotificationPopUp(message));
    }
